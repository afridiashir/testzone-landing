...
import { 
  MapPin, Home, Droplet, FileText, 
  Target, Zap, Microscope 
} from "lucide-react";
      ...
      <div className="container mx-auto px-4 flex flex-col md:flex-row items-center relative z-10">
        ...
        <div className="md:w-3/5 space-y-6">
          ...
          <p className="text-blue-100/80 text-lg max-w-xl font-light">
            Quality-assured pathology across 100+ cities, with results you and your physician can act on with confidence.
          </p>
          ...
          <div className="flex flex-wrap items-center gap-8 pt-10 mt-6 border-t border-blue-800/50">
            {[{num: "150+", label: "Branches"}, {num: "100+", label: "Cities"}, {num: "350K+", label: "Patients / Year"}, {num: "24/7", label: "Availability"}].map((stat, i) => (
              <div key={i} className="group cursor-default flex items-center gap-8">
                <div>
                  <h4 className="text-3xl font-bold group-hover:text-green-400 transition-colors">{stat.num}</h4>
                  <p className="text-[10px] text-blue-200/60 uppercase tracking-widest mt-1 group-hover:text-blue-200 transition-colors">{stat.label}</p>
                </div>
...
export function QuickFeatures() {
  const features = [
    { icon: MapPin, title: "Find a Lab", desc: "Locate your nearest collection center among 83 nationwide.", link: "Find Centers →", color: "text-rose-500", bg: "bg-rose-50" },
    { icon: Home, title: "Free Home Sampling", desc: "Book doorstep collection - free, nationwide, at your convenience.", link: "Book Now →", color: "text-blue-500", bg: "bg-blue-50" },
    { icon: Droplet, title: "Blood Bank Services", desc: "PBTA-approved blood transfusion services (Reg. 1429).", link: "Learn More →", color: "text-red-500", bg: "bg-red-50" },
    { icon: FileText, title: "Lab Test Directory", desc: "Browse our full menu of routine and specialized diagnostic tests.", link: "View Directory →", color: "text-indigo-500", bg: "bg-indigo-50" },
  ];
    ...
    <section className="container mx-auto px-4 -mt-24 relative z-20 pb-20">
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {features.map((feature, idx) => (
          ...
          >
            <div className={`mb-6 p-4 rounded-2xl ${feature.bg} ${feature.color} group-hover:scale-110 transition-transform duration-300`}>
              <feature.icon size={32} strokeWidth={1.5} />
            </div>
            <h3 className="font-bold text-[#1a2b56] text-xl mb-3">{feature.title}</h3>
            <p className="text-sm text-slate-500 mb-8 flex-grow leading-relaxed px-2">{feature.desc}</p>
            <a 
              href="#" 
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="text-center max-w-3xl mx-auto mb-12">
          ...
          <p className="text-slate-600 text-base leading-relaxed">
            Test Zone Diagnostic Centre delivers quality-assured pathology across 100+ cities. Our fully automated laboratories, staffed by 250+ trained professionals, combine international-standard technology with fast, dependable reporting.
          </p>
        </div>
        ...
        <div className="flex flex-wrap justify-center items-center gap-8 md:gap-16 border-t border-slate-100 pt-12">
          {stats.map((stat, i) => (
            <div key={i} className="text-center flex gap-8 md:gap-16 items-center">
              <div>
                <h4 className="text-3xl md:text-4xl font-extrabold text-[#1a2b56] mb-1">{stat.num}</h4>
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {depts.map((dept, i) => (
            <div key={i} className="bg-white rounded-xl overflow-hidden shadow-sm border border-slate-200">
              ...
              <div className="p-6">
                ...
                <p className="text-sm text-slate-600 mb-4">Comprehensive testing and analysis for accurate diagnosis.</p>
                <a href="#" className="text-[#5bc55e] text-sm font-medium hover:underline">Read more &rarr;</a>
              </div>
            </div>
          ))}
          ...
          <div className="bg-[#1a2b56] rounded-xl p-8 flex flex-col items-center justify-center text-center text-white">
            ...
            <p className="text-blue-200 mb-6">Browse our complete list of diagnostic tests and profiles.</p>
            <Button variant="outline" className="border-white bg-transparent hover:bg-white hover:text-[#1a2b56]">
              View Directory
            </Button>
          </div>
        </div>
    ...
    <section className="py-20 bg-white">
      <div className="container mx-auto px-4">
        <div className="text-center mb-12">
          <h2 className="text-3xl font-bold text-slate-900">Certified Technologists & Specialists</h2>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {roles.map((role, i) => (
            <div key={i} className="p-6 border border-slate-200 rounded-lg hover:shadow-md transition cursor-default">
      ...
      <div className="container mx-auto px-4">
        <div className="flex flex-col md:flex-row items-center gap-12 mb-16">
          <div className="md:w-1/2">
            <img src={accreditationsImg} alt="Accreditation Certificates" className="rounded-lg shadow-lg w-full" />
          </div>
          <div className="md:w-1/2">
            <h2 className="text-3xl font-bold text-slate-900 mb-4">Accredited, Audited & Trusted</h2>
            <p className="text-slate-600 mb-6">
              Our facilities are fully certified by national health boards and international quality control organizations, ensuring 100% precision in every test.
            </p>
            <div className="flex flex-wrap gap-3">
              <span className="px-4 py-2 bg-white border border-slate-200 rounded-full text-sm font-medium text-slate-700 shadow-sm">ISO 9001:2015</span>
              <span className="px-4 py-2 bg-white border border-slate-200 rounded-full text-sm font-medium text-slate-700 shadow-sm">Punjab Healthcare Commission</span>
            </div>
          </div>
        </div>
      </div>
